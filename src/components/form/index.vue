      ...
      <el-row :gutter="20">
        <el-col :span="item.span || 24" v-for="(item, i) in props.list" :key="i">
          <el-form-item :label="item.label" :prop="item.prop" @change="formChange(item.onChange, formData[item.prop])">
            ...
            <el-select :filterable="item.filterable" :remote="item.remote" :remote-method="item.remoteMethod" :multiple="item.multiple" @change="formChange(item.onChange, formData[item.prop])" class="w100" v-else-if="item.type === 'select'" v-model="formData[item.prop]"
            ...
            <el-date-picker @change="formChange(item.onChange, formData[item.prop])" :value-format="item.format" class="w100" v-else-if="item.type === 'date'" v-model="formData[item.prop]" :type="item.pickerType" range-separator="-" :placeholder="item.placeholder || '请选择时间'"
              :start-placeholder="item.startPlaceholder || '开始时间'" :end-placeholder="item.endPlaceholder || '结束时间'" />
            <el-time-picker @change="formChange(item.onChange, formData[item.prop])" :value-format="item.format" class="w100" v-else-if="item.type === 'time'" v-model="formData[item.prop]" :placeholder="item.placeholder || '请选择时间'" is-range range-separator="-"
              :start-placeholder="item.startPlaceholder || '开始时间'" :end-placeholder="item.endPlaceholder || '结束时间'" />
            ...
            <el-upload v-else-if="item.type === 'upload'" :limit="item.multiple ? item.limit : 1" :accept="item.accept" :auto-upload="false" :file-list="fileList[item.prop]" :ref="`upload_${item.prop}`" :on-exceed="onExcees" :on-change="selectFileCallBack" :on-remove="removeFile"
              list-type="picture">
...
<script setup>
  ...
  const {
    ...
    getFilePath,
    deepClone
  } = useGetGlobalProperties() // 读取全局函数/参数
